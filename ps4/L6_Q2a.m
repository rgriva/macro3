...
% Agora para o cálculo dos payoffs em mentir, consideramos que ômega é o
% vetor de zeros. O procedimento é exatamente o mesmo de acima.

Omeganull=zeros(1,N);
S=ones(1,N);
Pacientesnull(1:N)=zeros(1,N); 



for i=1:N-1
    Pacientesnull(i+1)=sum(Omeganull(1:i)); 
end

Impacientesnull=find(Omeganull==0);

% Poupança

for i=Impacientesnull
    S(i)=NewF(Pacientesnull(i)+1, i);
    S(i)=S(i)/(1+S(i));
end

% Logo, S representa a taxa de poupança do banco nesse caso.

Acumulonull=1;
Dotacao_Economianull=e*N;
Consumonull=Dotacao_Economianull*ones(1,N);

for i=1:N
    Acumulonull=Acumulonull*S(i);
    Consumonull(i)=Acumulonull*Dotacao_Economianull;
end

Pag_Impacientesnull=(1-S).*[Dotacao_Economianull,Consumonull(1:N-1)];

Pag_Pacientesnull=zeros(1,N);
if sum(Omeganull)>0
    Auxnull=Consumonull(N)*R/sum(Omeganull);
    Pag_Pacientesnull=Omeganull.*Auxnull;
end
    
Totalnull=Pag_Impacientesnull+Pag_Pacientesnull;

% Assim, para o cálculo dos payoffs em mentir,

P_Esperado_Mentir = 0; % Chute inicial

for k = 1:N
    for i = 1:N
        omega(1,N-i+1) = floor(mod(2^(k-1)/(2^(i-1)),2));
    end
    P_Esperado_Mentir = P_Esperado_Mentir + omega*u(Totalnull)';
...
% De onde se conclui que o indivíduo paciente não tem incentivos para
% correr, i.e., mentir e consumir em t=1 quando acredita que todos farão o
% mesmo.
