...
ibov_retorno = (ibov(2:end) - ibov(1:end-1))./ibov(1:end-1);
...
nbeta = 500;
nsigma = 500;
beta_grid = linspace(0.1,1,nbeta); 
beta_grid(nbeta) = 0.99;
...
diff_retorno = ibov_retorno - selic(2:end);
...
dist = abs(equity_premium_simulado - equity_premium*ones(size(Re)));
[a,b] = find(dist == min(min(dist)));
beta_optimal = beta_grid(a)
sigma_optimal = sigma_grid(b)

%% Plotando
figure('DefaultAxesFontSize',16); hold on
waterfall(beta_grid,sigma_grid,dist)
